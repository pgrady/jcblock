...
static bool check_blacklist( char *callstr )
{
  ...
  int reti;
   ...
   // Let's see if the number coming from the modem is a valid NANP number
   /* Compile regular expression */
   /* The North American Numbering Plan (NANP) specifies what telephone
      numbers are valid in the U.S., Canada, and certain Caribbean countries.
      This is normally expressed in their lanugage as NXX-NXX-XXXX where N
      represents the numbers 2-9 and X represents the numbers 0-9. In addition,
      the middle digit of the area code may not be a 9 (reserved for future
  ...
  */
  ...
  /* We'll now hold reti as a variable with global scope within this function
     ...
     file (no valid entries) will cause the regex to never cause a block.
  */
